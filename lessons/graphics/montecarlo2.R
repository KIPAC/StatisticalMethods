...
points(3.5, dchisq(3.5,3), pch=1)
axis(1)
axis(1, at=c(2,3.5), lab=c(expression(theta[0]), expression(theta[1])))
axis(2)
box()
#mtext(expression(theta[0]), 1, at=2.2, line=0.1)
arrows(1, 0, y1=0.11, code=3, length=0.1)
text(1.4, 0.055, 'q')
dev.off()

open.png('mc2_slice2.png', resolution=200, height=2.5, xmin=0.175, ymin=0.19, xmax=0.98)
par(xaxs='i', yaxs='i')
plot(1, 1, type='n', axes=F, xlim=c(0, 17), ylim=c(0, 0.265), xlab=expression(theta), ylab=expression(P(theta)))
segments(0, 0.11, x1=4.4, lty=2)
points(2, dchisq(2,3), pch=19)
points(2+1.2*(1:2), dchisq(2+1.2*(1:2), 3), pch=4, col=c(4,2))
points(pmax(0, 2-1.2*(1:2)), dchisq(pmax(0, 2-1.2*(1:2)), 3), pch=4, col=c(4,2))
points(3.5, dchisq(3.5,3), pch=1)
axis(1)
axis(1, at=c(2,3.5), lab=c(expression(theta[0]), expression(theta[1])))
axis(2)
...
arrows(1, 0, y1=0.11, code=3, length=0.1)
text(1.4, 0.055, 'q')
legend('topright', c('initial point', 'evaluations to bound slice', 'slice', 'new point'), inset=0, pch=c(19,4,NA,1), lty=c(NA,NA,2,NA), cex=0.8)
dev.off()
...
# chains not repo'd; from (not)parallel_chain_fit.py
ch = lapply(0:3, function(i) read.table(paste0('notparallel', i, '.txt'), head=F))
ch2 = lapply(0:3, function(i) read.table(paste0('parallel', i, '.txt'), head=F))
open.png('mc2_parallel_xy1.png', resolution=200, height=2.5, xmin=0.175, ymin=0.19, xmax=0.98, ymax=0.9)
plot(1, 1, type='n', xlim=c(-16,16), ylim=c(-16,16), xlab='x', ylab='y')
for (i in 1:length(ch)) points(ch[[i]][1:3000,], type='l', col=i+1)
mtext('Independent, adaptive chains', 3, 0.25)
dev.off()
open.png('mc2_parallel_xy2.png', resolution=200, height=2.5, xmin=0.175, ymin=0.19, xmax=0.98, ymax=0.9)
plot(1, 1, type='n', xlim=c(-16,16), ylim=c(-16,16), xlab='x', ylab='y')
for (i in 1:length(ch2)) points(ch2[[i]][1:3000,], type='l', col=i+1)
mtext('Communicating, adaptive chains', 3, 0.25)
dev.off()
open.png('mc2_parallel_trace1.png', resolution=200, height=3, width=5, xmin=0.1, ymin=0.33, xmax=0.98, ymax=0.95)
par(mfrow=c(2,1))
plot(1, 1, type='n', xlim=c(1,3000), ylim=c(-10,10), xlab='step', ylab='x')
for (i in 1:length(ch)) points(ch[[i]][,1], pch='.', col=i+1)
plot(1, 1, type='n', xlim=c(1,3000), ylim=c(-15,15), xlab='step', ylab='y')
for (i in 1:length(ch)) points(ch[[i]][,2], pch='.', col=i+1)
dev.off()
open.png('mc2_parallel_trace2.png', resolution=200, height=3, width=5, xmin=0.1, ymin=0.33, xmax=0.98, ymax=0.95)
par(mfrow=c(2,1))
plot(1, 1, type='n', xlim=c(1,3000), ylim=c(-10,10), xlab='step', ylab='x')
for (i in 1:length(ch2)) points(ch2[[i]][,1], pch='.', col=i+1)
plot(1, 1, type='n', xlim=c(1,3000), ylim=c(-15,15), xlab='step', ylab='y')
for (i in 1:length(ch2)) points(ch2[[i]][,2], pch='.', col=i+1)
dev.off()
