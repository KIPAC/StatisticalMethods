...
xx = seq(1.875, 3, len=1000)
f = function(x) dnorm(x, 2, 0.19) + 0.13*dnorm(x, 1.65, 0.05) + 0.2*dnorm(x, 1.5, 0.4)
ff = f(xx)
ff = ff / sum(ff) / (xx[2]-xx[1])
getdist.1d(list(mids=xx, counts=ff, density=ff))
probs = c(2.1521483, 0.5131601)

open.png('bayes_ci_limit.png', resolution=200, height=2.5, xmin=0.175, ymin=0.19, xmax=0.98)
par(xaxs='i', yaxs='i')
plot(1, 1, type='n', xlim=c(1.875,3.1), ylim=c(0,2.9), xlab=expression(theta), ylab=expression(P(group("",theta,"|")*data)), axes=F)
abline(h=probs, lty=2, col='gray50')
j = which(ff >= probs[2])
polygon(c(xx[j[1]], xx[j], xx[rev(j)[1]]), c(0, ff[j], 0), col=shading[2], border=NA)
j = which(ff >= probs[1] & xx < 1.75)
